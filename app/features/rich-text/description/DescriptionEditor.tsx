...
export function DescriptionEditor({ initialValue, error }: DescriptionEditorProps) {
  ...
  return <div className="sb-description-field">
    ...
    <div className={`sb-description-editor${error ? " sb-description-editor-error" : ""}`} aria-invalid={Boolean(error)}>
      <DescriptionToolbar editor={state.editor} htmlMode={state.htmlMode} previewOnly={state.previewOnly} onToggleMode={state.toggleMode} />
    ...
    <span id="sb-description-warning" className="sb-description-warning" role={state.compatibilityError ? "status" : undefined}>{state.compatibilityError}</span>
