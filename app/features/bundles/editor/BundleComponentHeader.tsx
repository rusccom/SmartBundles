...
export function BundleComponentHeader(props: BundleComponentHeaderProps) {
  ...
  return <header className="sb-component-header">
    <button type="button" className="sb-disclosure" aria-expanded={props.expanded}
      aria-controls={props.detailsId} aria-label={`${props.expanded ? "Collapse" : "Expand"} ${props.selector.productTitle}`}
      onClick={props.onToggle}><span className="sb-disclosure-icon" aria-hidden="true">
        {props.expanded ? "−" : "+"}
      </span></button>
    ...
    <span className="sb-component-meta">
      ...
      <span className="sb-component-price">{formatEditorPriceRange(props.selector.options.filter(({ allowed }) => allowed),
