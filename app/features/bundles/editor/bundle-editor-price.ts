...
export function formatEditorPriceRange(options: EditorOption[], currencyCode: string, locale: string): string {
  const prices = options.flatMap(({ displayPrice }) => numericPrice(displayPrice));
